...
var logDeprecation = function (name) {
    ...
    var message = "SweetAlert warning: \"".concat(name, "\" option has been ").concat(destiny, ".");
    if (replacement) {
        var subOptionText = (subOption) ? " \"".concat(subOption, "\" in ") : ' ';
        message += " Please use".concat(subOptionText, "\"").concat(replacement, "\" instead.");
    }
    var DOMAIN = 'https://sweetalert.js.org';
    if (link) {
        message += " More details: ".concat(DOMAIN).concat(link);
    }
    else {
        message += " More details: ".concat(DOMAIN, "/guides/#upgrading-from-1x");
    }
