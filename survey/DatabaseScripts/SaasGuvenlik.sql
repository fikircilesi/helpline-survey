...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_SoruGrup_CalismaAlaniId' AND object_id = OBJECT_ID('dbo.SoruGrup'))
    CREATE INDEX IX_SoruGrup_CalismaAlaniId ON dbo.SoruGrup(CalismaAlaniId);
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Soru_CalismaAlaniId' AND object_id = OBJECT_ID('dbo.Soru'))
    CREATE INDEX IX_Soru_CalismaAlaniId ON dbo.Soru(CalismaAlaniId);
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_CevapGrup_CalismaAlaniId' AND object_id = OBJECT_ID('dbo.CevapGrup'))
    CREATE INDEX IX_CevapGrup_CalismaAlaniId ON dbo.CevapGrup(CalismaAlaniId);
GO

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Cevap_CalismaAlaniId' AND object_id = OBJECT_ID('dbo.Cevap'))
...
BEGIN
    ...
    UPDATE dbo.SoruGrup SET CalismaAlaniId = @CalismaAlaniId WHERE CalismaAlaniId IS NULL;
    UPDATE dbo.Soru SET CalismaAlaniId = @CalismaAlaniId WHERE CalismaAlaniId IS NULL;
    UPDATE dbo.CevapGrup SET CalismaAlaniId = @CalismaAlaniId WHERE CalismaAlaniId IS NULL;
    UPDATE dbo.Cevap SET CalismaAlaniId = @CalismaAlaniId WHERE CalismaAlaniId IS NULL;
END;
GO
